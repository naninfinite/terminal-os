 ...
 * - Manage internal ME.OS windows (open/focus/move/minimize/close).
...
import type { MeOsAppId, MeOsDisplayMode, MeOsPersistedSnapshot, MeOsWindow, MeOsWindowTemplate } from './types';

const STORAGE_KEY = 'terminalOS.meos.v1.shell';
const STORAGE_VERSION = 1 as const;

const WINDOW_TEMPLATES: Record<MeOsAppId, MeOsWindowTemplate> = {
  home: {
    id: 'meos_home',
    title: 'HOME.EXE',
    appId: 'home',
...
const sanitizeWindow = (raw: unknown): MeOsWindow | null => {
  ...
  if (appId !== 'home' && appId !== 'about' && appId !== 'projects' && appId !== 'media') return null;
...
export const MeOsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const openApp = useCallback((appId: MeOsAppId) => {
