...
- you_messages
- you_rate_limits
and RPC function you_allow_post. Edge Function exists as `you` exposing root `GET` + `POST`.
...
2) Create a small YouApiClient module that:
   - uses VITE_YOU_API_BASE_URL when set (Supabase direct), otherwise uses same-origin /api/you (future proxy).
