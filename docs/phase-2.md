...
- `ME` panel: portfolio window overlay (fullscreen modal) with media gallery + preview (images/videos), keyboard navigation, and reduced-motion friendly open/close
...
- `src/components/ME/` — portfolio “OS-like” window:
  - Panel view shows a small launcher (`OPEN`).
  - Opening renders a fullscreen overlay (`role="dialog"`) styled with the same CRT + scanlines layers as the rest of the app.
  - Includes a media gallery and preview area supporting images and videos, with arrow-key selection and `Esc` to close.
...
- Keyboard: `ME` overlay supports arrow keys to switch items and `Esc` to close; focus is moved into the overlay on open and restored on close.
